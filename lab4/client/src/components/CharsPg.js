...
const Chars = () => {
    ...
    useEffect(() => {
        async function dataFetch() {
            ...
            try {
                const { data } = await axios.get('http://localhost:4000/api/characters/page/'+(parseInt(pagenum)-1));
    ...
    if(chdata){
        Cards = chdata.results.map( (x) => {return buildCard2(x)});
    }
